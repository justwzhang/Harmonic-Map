...
namespace MeshLib
{
    ...
    inline void CHarmonicMapVertex::_to_string()
    {
        ...
        parser._removeToken("uv"); // intepreted as newToken "uv"
        //parser._removeToken("rgb");
        parser._toString(m_string);
        std::stringstream iss;
        //  iss << "rbg=(" << m_rgb[0] << " " << m_rgb[1] << " " << m_rgb[2] << ")";
        iss << " " << "uv=(" << m_uv[0] << " " << m_uv[1] << ")";
        m_string += iss.str();

        CParser parser2(m_string);
        parser2._removeToken("rgb"); // intepreted as newToken "uv"
        //parser._removeToken("rgb");
        parser2._toString(m_string);
        std::stringstream iss2;
        //  iss << "rbg=(" << m_rgb[0] << " " << m_rgb[1] << " " << m_rgb[2] << ")";
        iss2 << " " << "rgb=(" << m_rgb[0] << " " << m_rgb[1] <<" " << m_rgb[2] << ")";
        m_string += iss2.str();
    ...
    inline void CHarmonicMapMesh::reset_idx()
    {
        size_t idx = 0;
        for (CHarmonicMapMesh::MeshVertexIterator viter(this); !viter.end(); ++viter)
        {
            auto pV = *viter;
            pV->idx() = idx++;
        }
