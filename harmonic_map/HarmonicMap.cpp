...
void MeshLib::CHarmonicMap::map()
{
    ...
    for (M::MeshVertexIterator viter(m_pMesh); !viter.end(); ++viter)
    {
        ...
        for (M::VertexVertexIterator witer(pV); !witer.end(); ++witer)
        {
            ...
            if (pW->boundary()) {//in the boundary then put into
...
void MeshLib::CHarmonicMap::_calculate_edge_weight()
{
    ...
    for (M::MeshFaceIterator fiter(m_pMesh); !fiter.end(); ++fiter)
    {
        ...
        // theta = acosine ((a^2+b^2-c^2)/2ab)
        int counter = 0;
        for (M::FaceHalfedgeIterator heiter(pF); !heiter.end(); ++heiter) {
            pH[counter++] = *heiter;
        }
        for (int i = 0; i < 3; i++) {
            //loop through the pH to calculate all angles
            M::CEdge* pEc = (M::CEdge*)pH[i%3]->edge();
            M::CEdge* pEa = (M::CEdge*)pH[(i+1)%3]->edge();
            M::CEdge* pEb = (M::CEdge*)pH[(i+2)%3]->edge();
            double a = pEa->length();
            double b = pEb->length();
            double c = pEc->length();

            pH[i]->angle() = _inverse_cosine_law(a, b, c);
    ...
    for (M::MeshEdgeIterator eiter(m_pMesh); !eiter.end(); ++eiter)
    {
        ...
        if (!pE->boundary()) {// if it is not a boundary edge
            M::CHalfEdge* halfedge1 = m_pMesh->edgeHalfedge(pE, 0);
            M::CHalfEdge* halfedge2 = m_pMesh->edgeHalfedge(pE, 1);
            pE->weight() = (1 / tan(halfedge1->angle())) + (1 / tan(halfedge2->angle()));

        }
        else { // it is a boundary edge
            M::CHalfEdge* halfedge1 = m_pMesh->edgeHalfedge(pE, 0);
            pE->weight() = 1 / tan(halfedge1->angle());
        }
